				...
				if (Bundlepath != nullptr && Bundlepath[0] != '\0')
				{
					...
					if (CurrentPKG != "")
					{
						...
						if (CurrentCAFF != "")
						{
							...
							//update tab based on file path ie "aid" / "texture" / "wild"
							//split currentaid by "_" and add them to a vector
							std::vector<std::string> split;
							std::string token;
							std::istringstream tokenStream(CurrentAid);
							...
							for (std::string s : split)
							{
								...
								{
									//aidpath = everything before s plus s seperated by "_"
									std::string aidpath = "";
									for (std::string s2 : split)
									{
										if (s2 != s)
										{
											aidpath += s2 + "_";
									...
									std::cout << "Selected: " << aidpath << std::endl;
								}
								ImGui::PopID();
							}
							...
							if (CurrentAid == "") {
								...
								for (std::string c : pkg.VREFs[0].ChunkNames)
								{
									//get first aid before "_"
									std::string aid = c.substr(0, c.find_first_of("_"));
									//if aid is in aids vector then skip
									if (std::find(Aids.begin(), Aids.end(), aid) == Aids.end())
									{
										Aids.push_back(aid);
									}
								...
								//List files in CurrentAid path, If current path is aid_texture and there is a file that has aid_texture_madeup_dragon_test then it would display madeup
								std::vector<std::string> Aids;
								for (std::string c : pkg.VREFs[0].ChunkNames)
								{
									//subtract "CurrentAid_" from the c
									std::string aid = c.substr(CurrentAid.size() + 1, c.find_first_of("_", CurrentAid.size() + 1) - CurrentAid.size() - 1);
									//if aid is in aids vector then skip
									if (std::find(Aids.begin(), Aids.end(), aid) == Aids.end())
									{
										Aids.push_back(aid);
								...
								for (std::string a : Aids)
								{
									//id
									std::string tk = a;
									std::string id = CurrentAid + "_" + tk;

									std::cout << CurrentAid + "_" + tk << std::endl;

									ImGui::PushID(id.c_str());
									if (ImGui::ImageButton(m_Unknown_Thumbnail.GetDescriptorSet(), ImVec2(50, 50))) {
										CurrentAid = CurrentAid + "_" + tk;
									}
									ImGui::SameLine();
